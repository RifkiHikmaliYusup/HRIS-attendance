...
# URL API absensi
API_URL = "https://dev.hrd.asmat.app/api/v2/absen"
Regis_URL = "https://dev.hrd.asmat.app/api/v2/face-recognition/register"
            ...
            for entry in data:
                if 'nrp' in entry and 'encodings' in entry:
                    face_data[entry['nrp']] = {
                        ...
                        'name': entry.get('name', '')  # Ambil name dari data
                    }
                else:
...
def recognize_face(img, main_content):
    ...
    for face in faces:
        ...
        # Bandingkan wajah dengan data yang sudah disimpan
        ...
        if not matched_nrps:
            ...
            print("[DEBUG] Wajah tidak dikenal, disimpan sebagai Unknown")
        else:
            ...
            for matched_nrp in matched_nrps:
                ...
                if matched_nrp not in last_seen:
                    ...
                    print(f"[DEBUG] Kirim pertama kali NRP: {matched_nrp}")
                    ...
                    if current_time - last_seen[matched_nrp] >= 5:
                        ...
                        print(f"[DEBUG] Kirim ulang NRP: {matched_nrp}")
                        main_content.send_to_server(matched_nrp, image_path_local)
...
class MainContent(BoxLayout):
    ...
    def show_absen_notif(self, nrp, status, waktu):
        # Tampilkan name bukan NRP
        name = face_data.get(nrp, {}).get('name', nrp)  # Ambil name, jika tidak ada gunakan NRP
        self.absen_notif_label.text = f"[b]{name}[/b]\n{status}\n{waktu}"
    ...
    def update_location(self, **kwargs):
        ...
        # Dapatkan name jalan berdasarkan latitude dan longitude
    ...
    def back_to_main(self, instance):
        ...
        self.__init__(self.screen_manager) # Sesuaikan dengan name halaman utama yang benar
        self.on_enter()
        ...
        if self.sync_logo.collide_point(*touch.pos):
            ...
            while True:
                ...
                if isinstance(payload, dict):
                    all_data.append(payload)        # kalau dict → append
                elif isinstance(payload, list):
                    all_data.extend(payload)        # kalau list → extend
                ...
                if not all_data:
                    dialog = MDDialog(
                        text="[size=25]Data tidak ditemukan.[/size]",
            ...
            for item in all_data:
                ...
                try:
                    ...
                    print(f"⚠️ Error parsing face_encoding untuk {nrp}: {e}")
                    face_encoding = []
    ...
    def create_table(self):
        ...
        self.table = MDDataTable(
            ...
            row_data=[
                (f"[size=21]{name}[/size]", f"[size=21]{status}[/size]", f"[size=21]{waktu}[/size]")
    ...
    # Ambil pesan dari server
        message = response_data.get("message", "Unknown Status") if response_data else "Unknown Status"

        # Ambil data detail dari response
        data = response_data.get("data") if response_data else None
        if isinstance(data, dict):
            waktu_full = data.get("waktu", "Unknown Time")
            name = data.get("name", nrp)  # fallback ke NRP kalau nama tidak ada
        else:
            waktu_full = "Unknown Time"
            name = nrp

        # Coba parsing datetime
        try:
        ...
        # Tambahkan ke tabel + tampilkan notifikasi sesuai status
        if " sudah check" in message.lower():
            self.add_to_table(name, message, waktu)
            self.show_absen_notif(name, message, waktu)
                    ...
                    if classNames[cls] == 'real':
                        ...
                        for nrp, (x1, y1, _) in valid_faces.items():
                            if nrp == "Unknown":
                                ...
                                name = face_data.get(nrp, {}).get('name')
                                label = f"Nama: {name}"
                                color = (0, 255, 0)
        ...
        def thread_task():
            ...
            with open(image_path_local, "rb") as image_file:
                ...
                try:
                    ...
                    Clock.schedule_once(lambda dt, err=e: self.show_server_error_dialog(f"Error saat mengirim data ke server: {err}"))
...
class Registration(Screen):
    ...
    def save_face_data(self):
        ...
        try:
            ...
            # Cek status dari server (bukan hanya HTTP code)
            if resp_json.get("status") != 200:
                self.show_failed_dialog(f"Gagal registrasi. Pesan server: {resp_json.get('message')}")
                return

            # Ambil name kalau ada
            name = resp_json.get("name", nrp)
        ...
        database.append({"nrp": nrp, "name": name, "encodings": mean_encoding})
        with open(database_json, "w") as file:
            json.dump(database, file, indent=4)
        print(f"✅ Data wajah {nrp} berhasil disimpan di {database_json}")

        database_joblib_data.append({"nrp": nrp, "name": name, "encodings": mean_encoding})
